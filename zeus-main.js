...
const config = {
  ...
  if(username !== undefined) {
    var urlRepl = `https://${process.env.REPL_SLUG}.${username}.repl.dev`;
...
try {
    ...
    else logger.loader("Không thể load file config.json", "error");
}
...
catch { logger.loader("Không thể load file config.json", "error") }
    ...
    if (process.env.KEY && global.config.encryptSt) {
      ...
      writeFileSync(appStateFile, loginState)
        ...
        global.client.api = loginApiData
          logger(`Đang tiến hành kết nối đến server của Zeus Project`);
     logger.loader(`Kết nối thành công đến server của Zeus Project`)
