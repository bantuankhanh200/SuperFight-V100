        ...
        switch (logMessageType) {
            case "log:thread-admins": {
                if (logMessageData.ADMIN_EVENT == "add_admin") {
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage(`==== [ Cập Nhật Nhóm ] ====\n,📝 - Đã cập nhật người dùng ${logMessageData.TARGET_ID} trở thành quản trị viên nhóm\n⏰Time: ${time}`, threadID, async (error, info) => {
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage(`==== [ Cập Nhật Nhóm ] ====\n📝 - Đã cập nhật người dùng ${logMessageData.TARGET_ID} trở thành thành viên\n⏰ - Time: ${time}`, threadID, async (error, info) => {
                        if (global.configModule[this.config.name].autoUnsend) {
                            await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));
                            return api.unsendMessage(info.messageID);
                        } else return;
            ...
            case "log:user-nickname": {
                ...
                if (global.configModule[this.config.name].sendNoti) api.sendMessage(`==== [ Cập Nhật Nhóm ] ====\n📝 - Đã cập nhật biệt danh của người dùng ${logMessageData.participant_id} thành: ${(logMessageData.nickname.length == 0) ? "tên gốc": logMessageData.nickname}\n⏰ - Time: ${time}`, threadID, async (error, info) => {
                    if (global.configModule[this.config.name].autoUnsend) {
                        await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));
                        return api.unsendMessage(info.messageID);
                    } else return;
            ...
            case "log:thread-name": {
                ...
                if (global.configModule[this.config.name].sendNoti) api.sendMessage(`==== [ Cập Nhật Nhóm ] ====\n📝 - Đã cập nhật tên nhóm thành ${dataThread.threadName}\n⏰ - Time: ${time}`, threadID, async (error, info) => {
                    if (global.configModule[this.config.name].autoUnsend) {
                        await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));
                        return api.unsendMessage(info.messageID);
                    } else return;
