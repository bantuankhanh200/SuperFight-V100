module.exports = function({ api, models, Users, Threads, Currencies }) {
  ...
  return async function({ event }) {
    ...
    const replyAD = '[ MODE ] - Chỉ admin mới dung được Bot';
    ...
    if (commandName.startsWith(PREFIX)) {
      if (!command) {
        ...
        if (checker.bestMatch.rating >= 0.5) {
          ...
          return api.sendMessage(global.getText("handleCommand", "commandNotExist", checker.bestMatch.target), time, threadID);
        }
      }
    }
    ...
    if (timestamps && timestamps instanceof Map && timestamps.has(senderID) && dateNow < timestamps.get(senderID) + expirationTime)
      ...
      return api.setMessageReaction('⏳', event.messageID, err => (err) ? logger('Đã xảy ra lỗi khi thực hiện setMessageReaction', 2) : '', !![]);
    var getText2;
