...
function startBot(message) {
    ...
    const child = spawn("node", ["--trace-warnings", "--async-stack-traces", "zeus-main.js"], {
        cwd: __dirname,
        stdio: "inherit",
        shell: true
    });
    ...
    child.on("error", function (error) {
        logger("Lỗi: " + JSON.stringify(error), "[ Bắt đầu  ]");
    });
};
...
}).catch(err => logger("Không thể kiểm tra cập nhật.", "[ CHECK UPDATE ]"));
